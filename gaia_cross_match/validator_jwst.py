"""
validator_jwst.py - Cross-image validation of Gaia-JWST cross-matches.
...
For each target, groups processed images by filter/camera, then:
  ...
  1. Writes per-image source_quality.csv annotating each matched source with:
       mag_normalized     = jwst_mag_st_gdc + cross_image_zp  (comparable across images)
  ...
  2. Writes a per-target cross_match_catalog.csv with one row per
     (gaia_source_id, filter_camera) pair:
       gaia_source_id, filter_camera, n_images, image_list, jwst_index_list,
       mag_norm_mean, mag_norm_std, mag_norm_mad, is_consistent

Usage:
    conda activate pymc_new
    python validator_jwst.py --target Fornax_dSph --data-dir ./data
"""
...
def load_image_data(image_dir, image_name):
    ...
    cal_paths      = glob.glob(os.path.join(image_dir, '*_cal.fits'))
    if not (os.path.exists(matched_path) and
            os.path.exists(transform_path) and cal_paths):
        return None

    matched   = pd.read_csv(matched_path)
    transform = pd.read_csv(transform_path, index_col='parameter')['value']

    with fits.open(cal_paths[0]) as h:
        h0 = h[0].header
        h1 = h[1].header
    ...
    has_stmag = ('jwst_mag_st_gdc' in matched.columns and
                 matched['jwst_mag_st_gdc'].notna().any())
...
def find_processed_images(target, data_dir):
    ...
    for root, dirs, files in os.walk(jwst_root):
        ...
        if f'{name}_cal_catalog.fits' in files and 'matched_gaia.csv' in files:
...
def has_valid_stmag(matched_df):
    return ('jwst_mag_st_gdc' in matched_df.columns and
            matched_df['jwst_mag_st_gdc'].notna().any())
    ...
    for i in range(len(names)):
        for j in range(i + 1, len(names)):
            ...
            df_a = (group[a]['matched']
                    [['gaia_source_id', 'jwst_mag_st_gdc']]
                    .rename(columns={'jwst_mag_st_gdc': 'mag_a'}))
            df_b = (group[b]['matched']
                    [['gaia_source_id', 'jwst_mag_st_gdc']]
                    .rename(columns={'jwst_mag_st_gdc': 'mag_b'}))
...
def validate_filter_group(group, ref_name, zp_dict, mag_scatter_thr, offset_tol_mag,
    ...
    zp_dict: {name: zp} pre-computed via propagate_zps_bfs so that
             mag_norm_i = jwst_mag_st_gdc_i + zp_i ≈ jwst_mag_st_gdc_ref.
             ref_name has zp = 0.0 by construction.

    All images in `group` must have jwst_mag_st_gdc (caller guarantees this).
    ...
    for name, d in group.items():
        zp = zp_dict[name]
        d['mag_norm']   = d['matched']['jwst_mag_st_gdc'].values + zp
        d['cross_zp']   = zp
        d['used_stmag'] = True

    # --- Cross-image source magnitude table ---
    # Error for mag_st_gdc equals jwst_mag_err_gdc (STMAG is a linear flux
    ...
    # aggressive.
    has_err = all('jwst_mag_err_gdc' in d['matched'].columns for d in group.values())
    rows = []
    for name, d in group.items():
        ...
        tmp['image']    = name
        if has_err:
            tmp['mag_err'] = d['matched']['jwst_mag_err_gdc'].values
        rows.append(tmp)
...
def write_solo_quality(data):
    ...
    if not has_valid_stmag(data['matched']):
        print(f"  WARNING: {data['image_name']} missing jwst_mag_st_gdc "
              f"(stale jwst1pass output?) — solo image skipped")
        return None
    mag_norm = data['matched']['jwst_mag_st_gdc'].values.copy()
...
def build_global_catalog(images, target, data_dir):
    ...
    for name, d in images.items():
        ...
        cols = ['gaia_source_id', 'jwst_index', 'image_name', 'filter_camera',
                'mag_normalized', 'mag_norm_mad', 'mag_norm_wmean', 'mag_norm_werr',
                'n_consistent', 'outlier_images', 'is_trustworthy',
                'has_gaia_pms', 'gaia_gmag', 'residual_sigma', 'is_star']
    ...
    def agg_group(g):
        ...
        if 'is_star' in g.columns:
            star_vals = g['is_star'].astype(bool)
        ...
        return pd.Series({
            ...
            'jwst_index_list':        ','.join(g['jwst_index'].astype(str).tolist()),
...
def validate_target(target, data_dir, mag_scatter_thr=0.1,
    ...
    for filt, names in sorted(by_filter.items()):
        # Drop images without jwst_mag_st_gdc before building the graph.
        missing = [n for n in names if not has_valid_stmag(images[n]['matched'])]
        if missing:
            print(f'\n  [{filt}] WARNING: {len(missing)} image(s) missing '
                  f'jwst_mag_st_gdc (stale jwst1pass?) — skipped: {missing}')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Cross-image validation of Gaia-JWST cross-matches.')
