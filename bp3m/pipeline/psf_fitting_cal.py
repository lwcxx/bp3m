"""
Step 3: PSF-fit each downloaded JWST CAL image using jwst1pass_py_v2.

JWST-specific. For HST FLC/FLT images use psf_fitting.py.

Calls jwst1pass_py_v2.io.run_photometry_fits for every image under
    {output_dir}/{field}/{telescope}/mastDownload/{telescope}/{obs_id}/
and writes
    {obs_id}_cal_catalog.fits
in the same directory.

Default parameters are tuned for JWST CAL images (NIRISS, NIRCam, MIRI).
...
def _fit_one_image(args):
    """Fit a single JWST CAL image. Thin wrapper around _fit_one_image_jwst.

    args is a 7-tuple: (image_path, out_catalog, lib_dir, params,
                        params_meta, verbose, psf_delta)
    psf_delta is accepted for API compatibility with _image_worker but
    is currently unused (JWST PSF iteration not yet wired).
    """
...
def _fit_one_image_jwst(image_path, out_catalog, lib_dir, params, params_meta, verbose):
    ...
    Produces the same set of outputs as _fit_one_image so that downstream
    ...
    try:
        ...
        # Same layout as _fit_one_image for HST so downstream tools are uniform.
        ...
        # Same logic as _fit_one_image.  psf_delta is always None for JWST (no
        # prior correction), so cumulative_delta equals the incremental measurement.
